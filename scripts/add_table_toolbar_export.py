# -*- coding: utf-8 -*-
"""
Adds PDF, Word and Excel export buttons right above the table header in Toplantilar Karar Ozeti
and implements unit-specific smart naming (e.g. 'Satın Alma Birimi — Toplantı Karar Özeti').
"""
import re
...
# 1. Update the table header right above the table in section-meetings-summary
old_summary_table_header = """                <div class="p-4 border-b border-slate-100 flex items-center justify-between">
...
new_summary_table_header = """                <div class="p-3.5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/50">
                    <div class="font-bold text-slate-800 text-xs flex items-center gap-2">
                        <div class="w-6 h-6 rounded-lg bg-indigo-600 text-white flex items-center justify-center text-xs shadow-2xs">
                            <i class="fa-solid fa-list-check"></i>
                        </div>
                        <span id="summaryTableHeaderTitle">Konsolide Karar ve Aksiyon Kayıtları</span>
                        <span class="bg-indigo-100 text-indigo-800 border border-indigo-200 px-2 py-0.5 rounded-full text-[10px] font-extrabold" id="summaryCountBadge">0 Kayıt</span>
                    </div>
                    <!-- 🆕 Seçim Yapılan Kısım Üstündeki İndirme Butonları (PDF, Word, Excel) -->
                    <div class="flex flex-wrap items-center gap-1.5 shrink-0">
                        <button onclick="exportDecisionsToPDF('summary')" class="bg-rose-600 hover:bg-rose-500 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-xs cursor-pointer hover:shadow-sm" title="Filtrelenen Kararları PDF Olarak İndir / Yazdır">
                            <i class="fa-solid fa-file-pdf text-rose-100"></i> PDF İndir
                        </button>
                        <button onclick="exportDecisionsToWord('summary')" class="bg-blue-600 hover:bg-blue-500 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-xs cursor-pointer hover:shadow-sm" title="Filtrelenen Kararları Word Belgesi (.doc) Olarak İndir">
                            <i class="fa-solid fa-file-word text-blue-100"></i> Word İndir
                        </button>
                        <button onclick="exportDecisionsToExcel('summary')" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-1.5 rounded-xl text-xs flex items-center gap-1.5 transition-all shadow-xs cursor-pointer hover:shadow-sm" title="Filtrelenen Kararları Excel Tablosu (.xls / .csv) Olarak İndir">
                            <i class="fa-solid fa-file-excel text-emerald-100"></i> Excel İndir
                        </button>
                    </div>
                </div>"""

if old_summary_table_header in html:
    html = html.replace(old_summary_table_header, new_summary_table_header)
    print("Summary table header updated with PDF, Word and Excel buttons!")
else:
    print("Warning: old_summary_table_header not matched directly, trying regex")
    html = re.sub(
        r'<div class="p-4 border-b border-slate-100 flex items-center justify-between">\s*<div class="font-bold text-slate-800 text-xs flex items-center gap-2">\s*<i class="fa-solid fa-list-check text-indigo-600"></i>\s*<span>Konsolide Karar ve Aksiyon Kayıtları</span>\s*<span class="bg-slate-100 text-slate-700 px-2 py-0\.5 rounded-full text-\[10px\] font-extrabold" id="summaryCountBadge">0 Kayıt</span>\s*</div>\s*</div>',
        new_summary_table_header,
        html
    )

# 2. Add Excel export button to the other views as well
# In İcra Kurulu
html = html.replace(
    ...
    """<button onclick="exportDecisionsToWord('icra', 'İcra Kurulu Toplantı Kararları ve Tutanağı')" class="bg-blue-600 hover:bg-blue-500 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer">
                    ...
                    <button onclick="exportDecisionsToExcel('icra', 'İcra Kurulu Toplantı Kararları ve Tutanağı')" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer">
...
html = html.replace(
    ...
    """<button onclick="exportDecisionsToWord('purchasing', 'Satınalma & Kategori Toplantı Kararları')" class="bg-blue-700 hover:bg-blue-600 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer">
                    ...
                    <button onclick="exportDecisionsToExcel('purchasing', 'Satınalma & Kategori Toplantı Kararları')" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer">
                        <i class="fa-solid fa-file-excel"></i> Excel İndir
                    </button>"""
)
...
html = html.replace(
    ...
    """<button onclick="exportDecisionsToWord('field', 'Saha & Satış Değerlendirme Toplantı Kararları')" class="bg-teal-700 hover:bg-teal-600 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer">
                    ...
                    <button onclick="exportDecisionsToExcel('field', 'Saha & Satış Değerlendirme Toplantı Kararları')" class="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-3 py-2 rounded-xl text-xs flex items-center gap-1.5 transition-colors shadow-xs cursor-pointer">
                        <i class="fa-solid fa-file-excel"></i> Excel İndir
                    </button>"""
)

# 3. Update getScopedDecisions and add exportDecisionsToExcel in JS
enhanced_export_js = """
        // =========================================================================
        // 🆕 KARARLARI PDF, WORD VE EXCEL DÖKÜMANI OLARAK DIŞA AKTARMA MOTORU
        // (Birim ve filtre seçimlerine göre özel başlık & dinamik veri üretir)
        // =========================================================================
        function getScopedDecisions(scope) {
            if (scope === 'icra') {
                ...
                // 'summary' - Seçilen Birim ve Filtreye Göre Akıllı Başlık
                ...
                // Başlık Üretimi: Birim seçilmişse doğrudan birim adını öne çıkar
                ...
                if (unit) {
                    ...
                    docTitle = `Konsolide Toplantılar Karar Özeti`;
                }

                return {
                    title: docTitle,
                    unitName: unit || 'Konsolide (Tüm Birimler)',
        ...
        function exportDecisionsToWord(scope, customTitle) {
            ...
            const rowsHtml = records.map((r, i) => `
                <tr style="border-bottom: 1px solid #d1d5db;">
                    ...
                    <td style="padding: 8px; font-weight: bold;">${r.unit}</td>
        ...
        function exportDecisionsToExcel(scope, customTitle) {
            ...
            // Excel HTML Format (Supported perfectly by Excel)
            const rowsHtml = records.map((r, i) => `
                <tr>
...
if start_idx != -1 and end_idx != -1:
    html = html[:start_idx] + enhanced_export_js + "\n\n" + html[end_idx:]
    print("Enhanced export functions with Excel support replaced!")
else:
    print("Warning: start/end markers not matched directly, updating via regex")
    html = re.sub(
...
print("Add table toolbar export complete!")
